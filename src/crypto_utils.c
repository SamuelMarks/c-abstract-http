...
int sha1_final(struct sha1_ctx* ctx, unsigned char out_hash[20]) {
    ...
    for (i = 0; i < 20; i++) {
        out_hash[i] = (unsigned char)((ctx->state[i >> 2] >> ((3 - (i & 3)) * 8)) & 255);
    }
...
int base64_encode(const unsigned char* in, size_t in_len, char** out_str, size_t* out_len) {
    size_t len;
    size_t i;
    size_t j;
    char *out;

    if (!out_str || !out_len || (!in && in_len > 0)) return -1;

    len = 4 * ((in_len + 2) / 3);
    out = (char*)malloc(len + 1);
    if (!out) return 12; /* ENOMEM fallback */
...
int base64_decode(const char* in, size_t in_len, unsigned char** out_data, size_t* out_len) {
    size_t i;
    size_t j;
    size_t out_size;
    unsigned char* out;

    if (!out_data || !out_len || (!in && in_len > 0)) return -1;
    if (in_len % 4 != 0) return 22; /* EINVAL fallback */

    out_size = in_len / 4 * 3;
    if (in_len > 0 && in[in_len - 1] == '=') out_size--;
    if (in_len > 1 && in[in_len - 2] == '=') out_size--;

    out = (unsigned char*)malloc(out_size);
    if (!out) return 12; /* ENOMEM fallback */

    for (i = 0, j = 0; i < in_len;) {
        uint32_t sextet_a = in[i] == '=' ? 0 : base64_decode_table[(unsigned char)in[i]]; i++;
        uint32_t sextet_b = in[i] == '=' ? 0 : base64_decode_table[(unsigned char)in[i]]; i++;
        uint32_t sextet_c = in[i] == '=' ? 0 : base64_decode_table[(unsigned char)in[i]]; i++;
        uint32_t sextet_d = in[i] == '=' ? 0 : base64_decode_table[(unsigned char)in[i]]; i++;
        ...
        if (j < out_size) out[j++] = (triple >> 2 * 8) & 0xFF;
        if (j < out_size) out[j++] = (triple >> 1 * 8) & 0xFF;
        if (j < out_size) out[j++] = (triple >> 0 * 8) & 0xFF;
    }
