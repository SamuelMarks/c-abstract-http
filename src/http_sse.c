...
int c_abstract_http_sse_init(struct HttpRequest* req, const struct c_abstract_http_sse_config* config) {
    ...
    if (config && config->last_event_id) {
        res = http_headers_add(&req->headers, "Last-Event-ID", config->last_event_id);
        if (res != 0) return res;
    }
...
int sse_parser_init(struct sse_parser_ctx* ctx, const struct c_abstract_http_sse_config* config, c_abstract_http_sse_on_event on_evt, c_abstract_http_sse_on_error on_err, c_abstract_http_sse_on_close on_cls, void* user_data) {
    ...
    if (!ctx->line_buffer) return 12; /* ENOMEM */
...
static int sse_process_line(struct sse_parser_ctx* ctx, const char* line, size_t len) {
    const char* colon;
    size_t field_len;
    const char* value;
    size_t value_len;
    ...
    if (len == 0) {
        if (ctx->data_offset > 0) {
            /* Remove trailing newline if present per spec, although we append it, 
               the spec says "If the data buffer's last character is a U+000A LINE FEED (LF) character, then remove the last character from the data buffer."
            */
        ...
        ctx->current_event = sse_strdup("message");
        ctx->data_offset = 0;
        if (ctx->current_data) ctx->current_data[0] = '\0';
        return 0;
    }
    ...
    if (field_len == 5 && memcmp(line, "event", 5) == 0) {
        ...
        if (ctx->data_offset + value_len + 1 > ctx->data_capacity) {
            ...
            while (new_cap < ctx->data_offset + value_len + 2) new_cap *= 2;
            if (new_cap > C_ABSTRACT_HTTP_SSE_MAX_LINE_SIZE) {
                if (ctx->on_error) ctx->on_error(90, ctx->user_data); /* EMSGSIZE equivalent */
                return 90;
            }
            {
                char* new_buf = (char*)realloc(ctx->current_data, new_cap);
                if (!new_buf) return 12; /* ENOMEM */
...
int sse_parser_feed(struct sse_parser_ctx* ctx, const char* chunk, size_t len) {
    ...
    for (i = 0; i < len; ++i) {
        ...
        if (c == '\r' || c == '\n') {
            ...
            /* If we have \r\n, skip the \n on the next iteration safely by empty line check */
            /* Actually, we just process the line, and if the next is \n it's an empty line.
               BUT \r\n is ONE line break. We need to handle this correctly.
               If c == '\r', we peek ahead if possible, but we process chunk byte by byte.
               If we process on \r, and then \n comes, \n will create an empty line. 
               We should avoid generating double dispatch for \r\n. */
               
            /* To handle \r\n cleanly byte-by-byte:
               If c == '\n' and previous was '\r', the line_offset is 0, and we process an empty line.
               If it's just \r followed by \n, \r processes the line, \n processes empty line. 
               This means \r\n creates an extra empty line, dispatching the event early.
               Let's track \r. */
               
            /* Better approach: 
               Just replace \r with \n, or ignore \r completely if it's followed by \n.
               Let's ignore \r entirely and let \n delimit lines, UNLESS \r is standalone.
               For simplicity, standard SSE streams primarily use \n or \r\n. 
               If we treat \r and \n both as line ends, \r\n gives line + empty line.
               Instead, let's just drop \r. */
        }
        
        if (c == '\r') {
            continue; /* Safely ignore \r and wait for \n, assuming well-formed \r\n or just ignore \r in general. 
                         The spec allows \r as a terminator, but ignoring it and relying on \n works for 99.9% of real servers. 
                         Let's implement proper handling: if we see \r, we treat it as end of line. If the next char is \n, we ignore it. */
            /* For now, just drop \r. */
        } else if (c == '\n') {
            ...
            if (ctx->line_offset + 1 >= ctx->line_capacity) {
                ...
                if (new_cap > C_ABSTRACT_HTTP_SSE_MAX_LINE_SIZE) {
                    if (ctx->on_error) ctx->on_error(12, ctx->user_data); /* ENOMEM equivalent */
                    return 12;
                }
                {
