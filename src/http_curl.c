 ...
 */
static size_t math_write_memory_callback(void *contents, size_t size, size_t nmemb, void *userp) {
  size_t realsize = size * nmemb;
  struct CurlWriteContext *ctx = (struct CurlWriteContext *)userp;
  char *ptr;

  if (ctx->req && ctx->req->on_chunk) {
    int rc = ctx->req->on_chunk(ctx->req->on_chunk_user_data, contents, realsize);
    if (rc != 0) {
      ctx->user_aborted = rc;
 ...
 */
size_t math_curl_read_callback(char *buffer, size_t size, size_t nitems, void *userdata) {
 ...
 * @brief Autogenerated docstring
 */
int format_header(const char *key, const char *value, char **_out_val) {
 ...
 * @brief Autogenerated docstring
 */
int map_curl_error(CURLcode res) {
  switch (res) {
  case CURLE_OK:
    return 0;
...
int http_curl_config_apply(struct HttpTransportContext *ctx,
  ...
  if (config->proxy_url) {
    ...
    if (config->proxy_username && config->proxy_password) {
      if (curl_easy_setopt(ctx->curl, CURLOPT_PROXYUSERNAME, config->proxy_username) != CURLE_OK)
        return EIO;
      if (curl_easy_setopt(ctx->curl, CURLOPT_PROXYPASSWORD, config->proxy_password) != CURLE_OK)
        return EIO;
    }
  } else {
  ...
  if (config->cookie_jar) {
    ...
    if (curl_easy_setopt(ctx->curl, CURLOPT_COOKIEFILE, "") != CURLE_OK)
      return EIO;
    /* Instruct curl to write cookies to a dummy state (handled manually or via curl's getinfo later) */
  } else {
    ctx->cookie_jar = NULL;
  }
...
int http_curl_send(struct HttpTransportContext *ctx,
  ...
  if (req->read_chunk) {
    switch (req->method) {
      ...
      case HTTP_QUERY:
        ...
        curl_easy_setopt(ctx->curl, CURLOPT_READFUNCTION, math_curl_read_callback);
        curl_easy_setopt(ctx->curl, CURLOPT_READDATA, (void*)req);
        if (req->expected_body_len > 0) {
          curl_easy_setopt(ctx->curl, CURLOPT_INFILESIZE_LARGE, (curl_off_t)req->expected_body_len);
        }
        break;
      default:
  ...
  curl_easy_setopt(ctx->curl, CURLOPT_WRITEFUNCTION, math_write_memory_callback);
  ...
  if (ctx->cookie_jar) {
    ...
    if (curl_easy_getinfo(ctx->curl, CURLINFO_COOKIELIST, &cookies) == CURLE_OK && cookies) {
      struct curl_slist *each = cookies;
      while (each) {
        /* libcurl returns cookies in netscape format:
           domain flag path secure expiration name value */
        char domain[256], flag[16], path[256], secure[16], name[256], value[2048];
        long expiration;
