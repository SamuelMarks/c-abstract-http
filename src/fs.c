...
int fs_is_directory(const char *path, int *out_is_dir) {
  c_stat st;
  if (!out_is_dir) return EINVAL;
  *out_is_dir = 0;
  if (!path) return EINVAL;
  if (c_stat_func(path, &st) != 0) return 0;
...
int path_is_unc(const char *path, int *out_is_unc) {
  if (!out_is_unc) return EINVAL;
  if (!path) { *out_is_unc = 0; return EINVAL; }
  *out_is_unc = (strlen(path) > 2 && path[0] == '\\' && path[1] == '\\') ? 1 : 0;
  return 0;
}
#endif /* _MSC_VER */
