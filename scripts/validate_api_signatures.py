...
def main():
    ...
    res = subprocess.run(
        ["ctest", "-C", "Debug", "--output-on-failure"], cwd="build-strict", env=env
    )
    if res.returncode != 0:
        print("Tests failed.")
        sys.exit(1)
    ...
    regex = re.compile(
        r"^\s*(extern\s+)?int\s+(http_|c_abstract_http_)[a-zA-Z0-9_]*\s*\("
    )
    ...
    if has_error:
        print(
            "ERROR: Found invalid API functions returning 'int' instead of 'enum c_abstract_http_error'."
        )
        sys.exit(1)
