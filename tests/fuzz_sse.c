...
static int dummy_on_event(const struct c_abstract_http_sse_event* ev, void* user_data) {
    (void)ev;
    (void)user_data;
    return 0;
}
...
int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size) {
    ...
    if (sse_parser_init(&ctx, NULL, dummy_on_event, dummy_on_error, dummy_on_close, NULL) != 0) {
        return 0;
    }
