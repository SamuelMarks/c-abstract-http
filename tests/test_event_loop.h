...
TEST test_event_loop_multiple_timers(void) {
  ...
  /* Add many timers and let them expire to test heap down */
  {
      ...
      int i;
      for (i = 0; i < 10; ++i) {
          ASSERT_EQ(0, http_loop_add_timer(loop, (10 - i) * 10, timer_dummy_cb, NULL, &ids[i]));
      }
      /* Run the loop for enough time to expire all of them. */
      /* Each tick pops the smallest. */
      /* Wait, we need to advance time or tick them! */
      /* Actually, `http_loop_run` or `http_loop_tick` doesn't exist? Oh it's `http_loop_tick`. */
      /* wait, does `http_loop_tick` advance time or use system time? It uses `c_cdd_http_clock_ms()`. */
...
TEST test_event_loop_heap_down(void) {
  ...
  for (i = 0; i < 10; ++i) {
      ...
      ASSERT_EQ(0, http_loop_add_timer(loop, timeout, timer_dummy_cb, NULL, &ids[i]));
  }
  ...
  /* Wait, they need to expire! But they have timeouts of 10, 20, 30... up to 200 ms.
     So we just need to sleep for 200ms and call tick! */
...
TEST test_event_loop_missing_hooks(void) {
  ...
  ASSERT_EQ(ENOTSUP, http_loop_add_timer(loop, 10, timer_dummy_cb, NULL, &timer_id));
  ASSERT_EQ(ENOTSUP, http_loop_cancel_timer(loop, 1));
  
  /* wakeup is a no-op if hook is missing, returns 0 */
  ASSERT_EQ(0, http_loop_wakeup(loop));
  
  /* tick returns 0 when there are hooks, even if missing `tick` hook, because there isn't one */
  ASSERT_EQ(0, http_loop_tick(loop));
...
TEST test_event_loop_fd_edges(void) {
  ...
  /* Now it's full (capacity=16, we added 1(fd=2)+15 = 16). Next add will realloc. */
  g_mock_alloc_fail = 1;
  g_mock_alloc_count = 0;
...
TEST test_event_loop_lazy_timer_cancel(void) {
  ...
  /* Tick should process the inactive timer from next_timeout loop,
     wait, if stop_requested is true, it returns before calculating next_timeout! */
  ASSERT_EQ(0, http_loop_tick(loop));
...
TEST test_event_loop_tick_fd_and_timer(void) {
  ...
  /* 552: next_timeout < 0 -> set to 0. 
     ...
     so `expiration - now < 0`. 
     If we add a timer for 1ms and sleep 10ms, then tick, it's processed in `process_timers`.
     Wait, `process_timers` REMOVES timers in the past!
     So when it calculates `next_timeout`, `loop->timers[0].expiration > now` ALWAYS!
     So `next_timeout` is ALWAYS >= 0!
     Wait! The current time `now` is taken AT THE BEGINNING of `http_loop_tick` (line 513).
     Then `process_timers` is called.
     Then `now` is UPDATED at line 542: `now = math_get_current_time_ms()`.
     If time advances between 513 and 542, `next_timeout` could be negative!
     Wait, how can we test this? Just mock time?
     Actually we don't mock time. We could just skip 552.
     Or wait! `process_timers` processes up to `now` (the time at start).
     Then we get `now` again. Time moves FORWARD!
     So `now` (new) >= `now` (old).
     If there was a timer at `now (old) + 1`, it was NOT processed.
     Then `now (new)` might be `now (old) + 2`.
     So `expiration (now_old+1) - now_new (now_old+2) = -1`!
     So it IS possible if we just sleep a tiny bit! But we can't easily inject a sleep between 513 and 542.
  */
  
  /* 573-584: setup fds in tick */
  /* We just need to add an fd and tick! */
  /* Why wasn't this covered? Because my `test_event_loop_tick_fd` used `http_loop_run` maybe? */
  
  ASSERT_EQ(0, pipe(pipefd));
  ASSERT_EQ(0, http_loop_add_fd(loop, pipefd[0], HTTP_LOOP_READ | HTTP_LOOP_WRITE | HTTP_LOOP_ERROR, mock_fd_cb, &triggered));
  ASSERT_EQ(0, http_loop_tick(loop));
  ...
  ASSERT_EQ(0, http_loop_add_timer(loop, 10, timer_cb_1, &triggered, &timer_id));
  ASSERT_EQ(0, http_loop_run(loop));
...
static void blocking_mock_fd_cb(struct ModalityEventLoop *loop, int fd, int revents, void *user_data) {
  int *triggered = (int *)user_data;
  (void)loop;
  (void)fd;
  (void)revents;
...
TEST test_event_loop_blocking_cb(void) {
  ...
  ASSERT_EQ(0, http_loop_add_fd(loop, pipefd[0], HTTP_LOOP_READ, blocking_mock_fd_cb, &triggered));
  write(pipefd[1], "a", 1);
...
TEST test_event_loop_mock_error_fd(void) {
  ...
  ASSERT_EQ(0, http_loop_add_fd(loop, pipefd[0], HTTP_LOOP_ERROR, mock_fd_cb, &triggered));
...
TEST test_event_loop_timeout_underflow(void) {
  ...
  /* process_timers calls time once, then tick calls it again.
     If we skip the jump on the first call, process_timers leaves it in the queue.
     Then tick jumps time forward, making expiration < now. */
  g_mock_time_jump = 1;
