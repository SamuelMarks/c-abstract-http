...
TEST test_ws_payload_too_large(void) {
  struct ws_parser_ctx parser;
  unsigned char chunk[] = { 0x81, 0x7f, 0x00, 0x00, 0x00, 0x00, 0x02, 0x00, 0x00, 0x00 };
  int rc;
  memset(&parser, 0, sizeof(parser));
  parser.on_error = test_ws_mock_on_error;
  test_ws_mock_on_error_called = 0;
...
TEST test_ws_masked_frame(void) {
  struct ws_parser_ctx parser;
  struct test_ws_ctx ctx = {0};
  unsigned char chunk[] = { 0x81, 0x85, 0x01, 0x02, 0x03, 0x04,
  ...
  if (parser.payload_buffer) free(parser.payload_buffer);
  PASS();
}
...
TEST test_ws_realloc_oom(void) {
  struct ws_parser_ctx parser;
  struct test_ws_ctx ctx = {0};
  unsigned char chunk[] = { 0x81, 10, '1','2','3','4','5','6','7','8','9','0' };
  int rc;
  memset(&parser, 0, sizeof(parser));
  parser.on_message = test_ws_on_message;
  parser.on_error = test_ws_mock_on_error;
...
TEST test_ws_parser_reassembly_too_large(void) {
  ...
  ASSERT_EQ(90, test_ws_mock_on_error_called);
  if (parser.payload_buffer) free(parser.payload_buffer);
  if (parser.reassembly_buffer) free(parser.reassembly_buffer);
  PASS();
}
...
TEST test_ws_sync_loop_init_oom(void) {
  ...
  g_mock_alloc_fail = 1;
  g_mock_alloc_count = 0;
  ASSERT_EQ(12, c_abstract_http_ws_sync_read_loop(&client, &req, test_ws_on_message, test_ws_on_error, test_ws_on_close, &ctx, NULL));
  g_mock_alloc_fail = 0;
  PASS();
}
...
TEST test_ws_sync_loop_parser_oom(void) {
  ...
  client.send = mock_send_success_ws;
  {
  /* The ws_init will do multiple allocations for headers. Let's just bypass it or count. 
     Actually, http_headers_add allocates for headers.
     ws_init calls http_headers_add 4 times. Each takes 2 allocations. So ~8 allocs.
     Let's just loop over g_mock_alloc_count to find where ws_parser_init fails. 
     Or simpler: just mock ws_parser_init to fail by finding the exact alloc count.
  */
  int i, rc;
  for (i = 0; i < 15; i++) {
      g_mock_alloc_fail = 1;
      g_mock_alloc_count = i;
      rc = c_abstract_http_ws_sync_read_loop(&client, &req, test_ws_on_message, test_ws_on_error, test_ws_on_close, &ctx, NULL);
      g_mock_alloc_fail = 0;
      http_request_free(&req);
      if (rc == 12) {
...
static int mock_send_bad_payload(struct HttpTransportContext *tctx, const struct HttpRequest *req, struct HttpResponse **res_out) {
...
TEST test_ws_sync_loop_feed_error(void) {
  ...
  test_ws_mock_on_error_called = 0;
  ASSERT_EQ(0, c_abstract_http_ws_sync_read_loop(&client, &req, test_ws_on_message, test_ws_mock_on_error, test_ws_on_close, &ctx, NULL));
  ASSERT_EQ(C_ABSTRACT_HTTP_ERR_WS_FRAMING, test_ws_mock_on_error_called);
