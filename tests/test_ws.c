...
TEST test_ws_pack_header_medium(void) {
    unsigned char buf[10];
    int len = ws_pack_header_medium(buf, 0, C_ABSTRACT_HTTP_WS_OPCODE_BINARY, 0, 1024);
    ASSERT_EQ(4, len);
...
TEST test_ws_pack_header_large(void) {
    unsigned char buf[10];
    int len = ws_pack_header_large(buf, 1, C_ABSTRACT_HTTP_WS_OPCODE_BINARY, 1, 65536);
    ASSERT_EQ(10, len);
...
static int test_ws_on_message(const struct c_abstract_http_ws_event* ev, void* user_data) {
...
TEST test_ws_parser_single_frame(void) {
    ...
    ws_parser_init(&parser, test_ws_on_message, test_ws_on_error, test_ws_on_close, &ctx);
...
TEST test_ws_sync_loop_exit_flag(void) {
    ...
    ASSERT_EQ(0, c_abstract_http_ws_sync_read_loop(NULL, NULL, NULL, NULL, NULL, NULL, &exit_flag));
    PASS();
}

TEST test_ws_async_register_stub(void) {
    ASSERT_EQ(-1, c_abstract_http_ws_async_register(NULL, NULL, NULL, NULL, NULL, NULL));
    PASS();
}
