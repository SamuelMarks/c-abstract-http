...
static int test_sse_on_event(const struct c_abstract_http_sse_event* ev, void* user_data) {
...
TEST test_sse_sync_loop_exit_flag(void) {
    ...
    ASSERT_EQ(0, c_abstract_http_sse_sync_read_loop(NULL, NULL, NULL, NULL, NULL, NULL, &exit_flag));
    PASS();
}

TEST test_sse_async_register_stub(void) {
    ASSERT_EQ(-1, c_abstract_http_sse_async_register(NULL, NULL, NULL, NULL, NULL, NULL));
    PASS();
}
