...
static void test_tls_task_cb(void *arg) {
  ...
  abstract_http_tls_set(tls_key, &thread_local_val);
...
TEST test_tls_isolation(void) {
  ...
  ASSERT_EQ(C_ABSTRACT_HTTP_SUCCESS, abstract_http_tls_key_create(&tls_key, NULL));
  ASSERT_EQ(C_ABSTRACT_HTTP_SUCCESS, abstract_http_thread_pool_init(&pool, 4));
  for (i = 0; i < 4; i++) {
    ASSERT_EQ(C_ABSTRACT_HTTP_SUCCESS, abstract_http_thread_pool_push(pool, test_tls_task_cb, &results[i]));
  }
  ...
  abstract_http_tls_key_delete(tls_key);
  tls_key = NULL;
  PASS();
}
...
TEST test_tls_oom(void) {
  ...
  struct AbstractHttpTlsKey *key = NULL;

  g_mock_alloc_fail = 1;
  g_mock_alloc_count = 0;
  rc = abstract_http_tls_key_create(&key, NULL);
  ...
  ASSERT_EQ(C_ABSTRACT_HTTP_ERR_IO, abstract_http_tls_key_create(&key, NULL));
...
#if !defined(_WIN32)
  g_mock_pthread_fail = 1;
  ASSERT_EQ(C_ABSTRACT_HTTP_ERR_IO, abstract_http_tls_set(key, NULL));

  g_mock_pthread_fail = 0;
#endif

  abstract_http_tls_key_delete(key);
...
TEST test_tls_errors(void) {
  ...
  ASSERT_EQ(C_ABSTRACT_HTTP_ERR_INVAL, abstract_http_tls_get(NULL, NULL));
  ASSERT_EQ(C_ABSTRACT_HTTP_ERR_INVAL,
            abstract_http_tls_get((struct AbstractHttpTlsKey *)1, NULL));
  abstract_http_tls_key_delete(NULL);
  PASS();
}
