...
TEST test_winhttp_lifecycle(void) {
  ...
  ASSERT_EQ(C_ABSTRACT_HTTP_SUCCESS, http_winhttp_global_cleanup());
  PASS();
#else
...
TEST test_winhttp_config_usage(void) {
  ...
  ASSERT_EQ(C_ABSTRACT_HTTP_SUCCESS, http_winhttp_context_init(&ctx));
  {
...
TEST test_winhttp_send_chunked(void) {
  ...
  ASSERT_EQ(C_ABSTRACT_HTTP_SUCCESS, http_winhttp_global_init());
  ASSERT_EQ(C_ABSTRACT_HTTP_SUCCESS, http_winhttp_context_init(&ctx));
  {
  ...
  ASSERT_EQ(C_ABSTRACT_HTTP_SUCCESS, http_winhttp_config_apply(ctx, &config));
...
TEST test_winhttp_send_multi(void) {
  ...
  ASSERT_EQ(C_ABSTRACT_HTTP_SUCCESS, http_future_init(&f1));
  ASSERT_EQ(C_ABSTRACT_HTTP_SUCCESS, http_future_init(&f2));
